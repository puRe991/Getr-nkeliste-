...
import { createDrink, createUser, sendPushNotification } from '@/services/api'
...
const emptyDrink: DrinkFormValues = { name: '', price: 1.5, stock: 24, is_active: true, icon: '🥤', category: 'getraenk' }

export function DrinkCreateForm() {
  const queryClient = useQueryClient()
  const form = useForm<DrinkFormValues>({ resolver: zodResolver(drinkSchema), defaultValues: emptyDrink })
  const mutation = useMutation({ mutationFn: createDrink, onSuccess: async () => { form.reset(emptyDrink); toast.success('Produkt angelegt'); await queryClient.invalidateQueries({ queryKey: ['drinks'] }) }, onError: (error) => toast.error(error.message) })
  return <Card><CardHeader><CardTitle>Getränk oder Essen anlegen</CardTitle></CardHeader><CardContent><form className="grid gap-3" onSubmit={form.handleSubmit((values) => mutation.mutate(values))}><Select {...form.register('category')}><option value="getraenk">Getränk</option><option value="essen">Essen</option></Select><Input placeholder="Name" {...form.register('name')} /><Input placeholder="Icon" maxLength={4} {...form.register('icon')} /><Input type="number" step="0.01" placeholder="Preis" {...form.register('price', { valueAsNumber: true })} /><Input type="number" step="1" placeholder="Bestand" {...form.register('stock', { valueAsNumber: true })} /><label className="flex items-center justify-between rounded-xl border border-border p-3">Aktiv <Switch checked={form.watch('is_active')} onCheckedChange={(checked) => form.setValue('is_active', checked)} /></label><FormError message={form.formState.errors.name?.message ?? form.formState.errors.price?.message ?? form.formState.errors.stock?.message} /><Button disabled={mutation.isPending}>Speichern</Button></form></CardContent></Card>
}
...
export function BroadcastForm() {
  ...
  const mutation = useMutation({
    mutationFn: () => sendPushNotification({ title: title.trim(), body: body.trim() }),
    onSuccess: (result) => { setTitle(''); setBody(''); toast.success(`Gesendet an ${result.sent} von ${result.total} Geräten`) },
    onError: (error: Error) => toast.error(error.message),
  })
  return (
    <Card>
      <CardHeader><CardTitle>Push-Nachricht an alle senden</CardTitle></CardHeader>
      <CardContent className="grid gap-3">
        <Input placeholder="Titel" value={title} onChange={(e) => setTitle(e.target.value)} maxLength={80} />
        <Input placeholder="Nachricht" value={body} onChange={(e) => setBody(e.target.value)} maxLength={200} />
        <Button type="button" disabled={!title.trim() || !body.trim() || mutation.isPending} onClick={() => mutation.mutate()}>Senden</Button>
      </CardContent>
    </Card>
  )
}
